...
// copied from: https://github.com/microsoft/vs-streamjsonrpc/tree/main/test/StreamJsonRpc.Tests/Samples
    ...
    public class HttpClientMessageHandler : IJsonRpcMessageHandler
    {
        ...
        public async ValueTask<JsonRpcMessage?> ReadAsync(CancellationToken cancellationToken)
        {
            ...
            using (var sequence = new Sequence<byte>())
            {
                ...
                var asMemory = buffer.AsMemory();
                try
                {
                    int bytesRead;
                    while (true)
                    {
                        bytesRead = await responseStream.ReadAsync(asMemory, cancellationToken).ConfigureAwait(false);                                                
                        if (bytesRead == 0)
                        {
                            break;
                        }
